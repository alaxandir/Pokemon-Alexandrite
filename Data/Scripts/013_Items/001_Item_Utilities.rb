...
def pbRaiseEffortValues(pkmn, stat, evGain = 60, ev_limit = true)
  stat = GameData::Stat.get(stat).id
  return 0 if ev_limit && pkmn.ev[stat] >= 180
  evTotal = 0
  GameData::Stat.each_main { |s| evTotal += pkmn.ev[s.id] }
  evGain = evGain.clamp(0, Pokemon::EV_STAT_LIMIT - pkmn.ev[stat])
...
def pbNatureChangeItem(pkmn,nature,item,scene)
  if [pkmn.nature_for_stats, pkmn.nature].include?(nature)
    ...
    if scene.pbConfirm(_INTL("It might affect {1}'s stats.\nAre you sure you want to use it?",pkmn.name))
      ...
      scene.pbMessage(_INTL("\\l[5]\\ts[]{1}'s stats changed due to the effects of the {2}! \\n\\c[9]NOTE:\\c[2] This does not change the nature itself, only the stats.\\nThis is how it works in the official games.",pkmn.name,
