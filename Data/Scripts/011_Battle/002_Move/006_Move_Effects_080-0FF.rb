...
class PokeBattle_Move_089 < PokeBattle_Move
  def pbBaseDamage(baseDmg,user,target)
    return [(user.happiness*1/3).floor,1].max
  end
end
...
class PokeBattle_Move_08A < PokeBattle_Move
  def pbBaseDamage(baseDmg,user,target)
    return [((255-user.happiness)*1/3).floor,1].max
  end
end
...
def pbHiddenPower(pkmn)
  ...
  idxType = 0; power = 60
  types = []
  GameData::Type.each { |t| types.push(t.id) if !t.pseudo_type && ![:NORMAL, :SHADOW, :FAIRY].include?(t.id)}
  ...
  def initialize(battle,move)
    ...
    @flingPowers = {
      ...
      100 => [:HARDSTONE,:RAREBONE,
       ...
       80 => [:ASSAULTVEST,:DAWNSTONE,:DUSKSTONE,:HEAVYDUTYBOOTS,:ELECTIRIZER,:MAGMARIZER,
       ...
       60 => [:ADAMANTORB,:DAMPROCK,:GRISEOUSORB,:HEATROCK,:LUSTROUSORB,
              :MACHOBRACE,:ROCKYHELMET,:STICK,:TERRAINEXTENDER,:UTILITYUMBRELLA
             ],
       ...
       40 => [:EVIOLITE,:ICYROCK,:LUCKYPUNCH,:FAIRYEARRING
             ],
