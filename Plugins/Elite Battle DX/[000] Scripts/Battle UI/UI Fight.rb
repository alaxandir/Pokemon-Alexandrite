...
class FightWindowEBDX
  ...
  def initialize(viewport = nil, battle = nil, scene = nil)
    ...
    @background.y = @viewport.height + 68 #Graphics.height - @background.bitmap.height
    @background.z = 100
  ...
  def update
    @sel.visible = true
    if @showMega
      @megaButton.y = @viewport.height - 24 #10 if @megaButton.y > @viewport.height - @background.bitmap.height/2
