...
Events.OnTalkToFollower += proc {|pkmn,x,y,random_val|
    items = [:TINYMUSHROOM,:HEARTSCALE,:PEARL,:CHERIBERRY,:CHESTOBERRY,:PECHABERRY,:RAWSTBERRY,:ASPEARBERRY,
			 :LEPPABERRY,:ORANBERRY,:PERSIMBERRY,:LUMBERRY,:SITRUSBERRY,:POKETOY,:EXPCANDYXS,:EXPCANDYS,
			 :CHERIBERRY,:CHESTOBERRY,:PECHABERRY,:RAWSTBERRY,:ASPEARBERRY,:SILKSCARF,:SUPERPOTION,:REVIVE,
			 :LEPPABERRY,:ORANBERRY,:PERSIMBERRY,:LUMBERRY,:SITRUSBERRY,:POKETOY,:EXPCANDYXS,:EXPCANDYS,
			 :POKEBALL,:POKEBALL,:GREATBALL,:GREATBALL,:ULTRABALL,:PLAINPACK,:FISTPACK,:WINGPACK,:TOXICPACK,
			 :EARTHPACK,:BOULDERPACK,:BUZZPACK,:SPOOKYPACK,:METALPACK,:SEARPACK,:AQUAPACK,:LEAFPACK,:VOLTPACK,
			 :MINDPACK,:FROSTPACK,:DRAGONPACK,:DARKPACK,:HEARTPACK,:REDSHARD,:YELLOWSHARD,:BLUESHARD,:GREENSHARD,
			 :REDAPRICORN,:YELLOWAPRICORN,:BLUEAPRICORN,:GREENAPRICORN,:PINKAPRICORN,:WHITEAPRICORN,:BLACKAPRICORN,
			 :FULLHEAL,:BERRYJUICE,:FRESHWATER,:FRESHWATER,:FRESHWATER,:ETHER,:ETHER,:HEALTHWING,:MUSCLEWING,:RESISTWING,
...
# Specific message if the Pokemon is a bug type and the map's name is route 3
  if $game_map.name == "Shimmerwood Forest" && pkmn.hasType?(:BUG)
...
Events.OnTalkToFollower += proc {|pkmn,x,y,random_val|
  if $game_map.name == "Anthrophil Lab"
    $scene.spriteset.addUserAnimation(FollowerSettings::Emo_Normal,x,y)
    pbMoveRoute($game_player,[PBMoveRoute::Wait,20])
    messages = [
...
# Specific message if the map name has the players name in it ie the Player's House
Events.OnTalkToFollower += proc {|pkmn,x,y,random_val|
...
Events.OnTalkToFollower += proc {|pkmn,x,y,random_val|
  if $game_map.name == "Gold Coast" || $game_map.name == "Trident Cove Town" || $game_map.name == "Seafoam City" || $game_map.name == "Fortune City"
    $scene.spriteset.addUserAnimation(FollowerSettings::Emo_Happy,x,y)
    pbMoveRoute($game_player,[PBMoveRoute::Wait,20])
    messages = [
