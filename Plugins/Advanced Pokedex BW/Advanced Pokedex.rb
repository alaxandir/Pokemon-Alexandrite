...
class PokemonPokedexInfo_Scene
    ...
    def displaySubPage
      ...
      # Bottom text  
      textpos = [[@data.name,Graphics.width/2-106-16,line2Y,2,BASE_COLOR,SHADOW_COLOR]]
      if $Trainer.owned?(@species)
        textpos.push([
          _INTL("{1}/{2}",@subPage,@totalSubPages),Graphics.width-40,line2Y,1,BASE_COLOR,SHADOW_COLOR])
      end
      ...
      if(@data.type1==@data.type2)
        ...
        overlay.blt(typeBaseX-48,line2Y+6,@typebitmap.bitmap,type1rect)
        overlay.blt(typeBaseX+48,line2Y+6,@typebitmap.bitmap,type2rect)
      end
    ...
    def getEvolutionMessage(evolution, method, parameter)
      ...
      ret = case method
        ...
        when :SpDefOverSpAtk
            _INTL("{1} at level {2} and SpDEF > ATK",evoName,parameter)
        when :SpeedOverSpDef
            _INTL("{1} at level {2} and SPEED > SpDEF",evoName,parameter)
        when :SpAtkOverSpDef
            _INTL("{1} at level {2} and SpATK > SpDEF",evoName,parameter)
