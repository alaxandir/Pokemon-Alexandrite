...
class PokemonBagPartyPanel < SpriteWrapper
  ...
  def initialize(pokemon,index,viewport=nil)
    ...
    if @active   # Rounded panel
      @panelbgsprite.addBitmap("able","Graphics/Pictures/Bag Party/ptpanel_desel")
      @panelbgsprite.addBitmap("ablesel","Graphics/Pictures/Bag Party/ptpanel_sel")
      @panelbgsprite.addBitmap("fainted","Graphics/Pictures/Bag Party/ptpanel_faint")
      @panelbgsprite.addBitmap("faintedsel","Graphics/Pictures/Bag Party/ptpanel_faint_sel")
      @panelbgsprite.addBitmap("swap","Graphics/Pictures/Bag Party/ptpanel_move")
      @panelbgsprite.addBitmap("swapsel","Graphics/Pictures/Bag Party/ptpanel_move_sel")
      @panelbgsprite.addBitmap("swapsel2","Graphics/Pictures/Bag Party/ptpanel_move_sel")
  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      if !@pokemon.egg?
        ...
        if @pokemon.hp>0
          w = @pokemon.hp*98*1.0/@pokemon.totalhp
          w = 1 if w<1
          w = ((w/2).round)*2
          hpzone = 0
          ...
          @overlaysprite.bitmap.blt(6,62,@hpbar.bitmap,hprect)
        end
        # Draw status
        status = 0
        if @pokemon.fainted?
...
class PokemonBag_Scene
  ...
  def pbStartScene(bag,party,choosing=false,filterproc=nil,resetpocket=true)
    ...
    @sprites["hud"] = IconSprite.new(0,0,@viewport)
    @sprites["hud"].setBitmap("Graphics/Pictures/Bag Party/hud")
    ...
    @sprites["itemlist"] = Window_PokemonBag.new(@bag,@filterlist,lastpocket,204,48-8,314,72+ITEMSVISIBLE*32)
    ...
    @pindex = @sprites["itemlist"].pocket-1
  ...
  def pbRefreshIndexChanged
    ...
    pbDrawTextPositions(overlay,[
       [PokemonBag.pocketNames[@bag.lastpocket],297,12-6,2,POCKETNAMEBASECOLOR,POCKETNAMESHADOWCOLOR]
    ])
    # Draw slider arrows
    showslider = false
  ...
  def pbStartScreen
    ...
    loop do
      ...
      if cmdRead>=0 && command==cmdRead   # Read mail
        ...
        if useType==1 || useType==3 || useType==4 || useType==5 || useType==6
...
def pbBagUseItem(bag,item,scene,screen,chosen,bagscene=nil)
  ...
  if itm.is_machine?    # TM or HM
    ...
    pbMessage(_INTL("\\se[PC access]You booted up {1}.\1",itm.name)) {screen.pbUpdate}
    if !pbConfirmMessage(_INTL("Do you want to teach {1} to {2}?",movename,pkmn.name)) {screen.pbUpdate}
      return 0
    else
      if pkmn.egg?
