...
class ModularTitleScreen
  ...
  def initialize
	...
	# setting up aiurjordan splash text
    @sprites["aj"] = Sprite.new(@viewport)
    @sprites["aj"].bitmap = pbBitmap("Graphics/MODTS/logo4")
    @sprites["aj"].center!
    @sprites["aj"].x = @viewport.rect.width/2
    @sprites["aj"].x = ModularTitle::START_POS[0] if ModularTitle::START_POS[0].is_a?(Numeric)
    @sprites["aj"].y = @viewport.rect.height*0.96
    @sprites["aj"].y = ModularTitle::START_POS[1] if ModularTitle::START_POS[1].is_a?(Numeric)
    @sprites["aj"].z = 999
    @sprites["aj"].visible = false
    @fade = 8
  end
